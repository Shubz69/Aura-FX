/**
 * GET /api/cron/cboe-uk-twelvedata-ingest — Twelve Data Cboe Europe Equities UK (BCXE) category.
 */
...
module.exports = async (req, res) => {
  ...
  try {
    const out = await runCategoryIngest('cboe_europe_equities_uk', { maxTier, symbolLimit, includeGlobal });
    return res.status(200).json({ success: true, ...out });
  } catch (e) {
    console.error('[cron/cboe-uk-twelvedata-ingest]', e);
    return res.status(500).json({ success: false, message: e.message || 'cboe uk ingest failed' });
  }
};
