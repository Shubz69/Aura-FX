/**
 * GET /api/cron/cboe-au-twelvedata-ingest — Twelve Data Cboe Australia (CXAC) category.
 */
...
module.exports = async (req, res) => {
  ...
  try {
    const out = await runCategoryIngest('cboe_australia', { maxTier, symbolLimit, includeGlobal });
    return res.status(200).json({ success: true, ...out });
  } catch (e) {
    console.error('[cron/cboe-au-twelvedata-ingest]', e);
    return res.status(500).json({ success: false, message: e.message || 'cboe au ingest failed' });
  }
};
