/**
 * GET /api/cron/forex-twelvedata-ingest — Twelve Data FX reference datasets (market_state, EOD, /forex_pairs).
 */
...
module.exports = async (req, res) => {
  ...
  try {
    const out = await runCategoryIngest('forex', { maxTier, symbolLimit, includeGlobal });
    return res.status(200).json({ success: true, ...out });
  } catch (e) {
    console.error('[cron/forex-twelvedata-ingest]', e);
    return res.status(500).json({ success: false, message: e.message || 'forex ingest failed' });
  }
};
