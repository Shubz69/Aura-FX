/**
 * GET /api/cron/uk-twelvedata-ingest — Twelve Data UK category (reference + inherited equity datasets).
 */
...
module.exports = async (req, res) => {
  ...
  try {
    const out = await runCategoryIngest('uk_equities', { maxTier, symbolLimit, includeGlobal });
    return res.status(200).json({ success: true, ...out });
  } catch (e) {
    console.error('[cron/uk-twelvedata-ingest]', e);
    return res.status(500).json({ success: false, message: e.message || 'uk ingest failed' });
  }
};
