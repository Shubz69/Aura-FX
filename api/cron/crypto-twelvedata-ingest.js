/**
 * GET /api/cron/crypto-twelvedata-ingest — Twelve Data crypto reference datasets (market_state, EOD, /cryptocurrencies).
 */
...
module.exports = async (req, res) => {
  ...
  try {
    const out = await runCategoryIngest('crypto', { maxTier, symbolLimit, includeGlobal });
    return res.status(200).json({ success: true, ...out });
  } catch (e) {
    console.error('[cron/crypto-twelvedata-ingest]', e);
    return res.status(500).json({ success: false, message: e.message || 'crypto ingest failed' });
  }
};
