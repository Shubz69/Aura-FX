...
function briefKindsRawLowerSet(rows) {
...
function isIntelAutomationPackComplete(rows, period) {
  const kinds = briefKindsRawLowerSet(rows);
...
/** Rows returned to the Market Intelligence UI: eight PDF sleeves only (+ Sunday open on daily). */
function institutionalKindsForIntelApiResponse(period) {
  const out = new Set(
    (period === 'weekly' ? INSTITUTIONAL_WEEKLY_WFA_KINDS : INSTITUTIONAL_DAILY_WFA_KINDS).map((k) =>
      String(k).toLowerCase()
    )
  );
  if (period === 'daily') out.add('aura_sunday_market_open');
  return out;
}
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    if (isIntel) {
      ...
      const intelResponseKinds = institutionalKindsForIntelApiResponse(period);
      const rowsInstitutionalOnly = (rows || []).filter((r) =>
        intelResponseKinds.has(String(r?.brief_kind || '').toLowerCase())
      );
      const packIncomplete =
        rowsInstitutionalOnly.length < expectedRows || !isIntelAutomationPackComplete(rows, period);
      ...
      const briefs = rowsInstitutionalOnly.map((r) => ({
        id: r.id,
        title: r.title || 'Brief',
      ...
      const haveKindRaw = briefKindsRawLowerSet(rows);
      ...
      const institutionalPdfComplete = expectedInstKinds.every((k) =>
        haveKindRaw.has(String(k).toLowerCase())
      );
      const payload = {
        success: true,
        briefs,
        ...
        categorySleevePack: {
          ...
          loaded: expectedInstKinds.filter((k) => haveKindRaw.has(String(k).toLowerCase())).length,
          missingKinds: expectedInstKinds.filter((k) => !haveKindRaw.has(String(k).toLowerCase())),
