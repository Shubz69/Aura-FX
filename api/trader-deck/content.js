...
/** In-memory throttle so partial-pack GETs can trigger gap-fill without stampeding the model on every refresh. */
const INTEL_GAP_FILL_COOLDOWN_MS = 3 * 60 * 1000;
const intelGapFillCooldown = new Map();

function briefKindsSet(rows) {
  return new Set((rows || []).map((r) => String(r?.brief_kind || '').toLowerCase()));
}

/** True when all eight category sleeves plus the institutional brief exist for this period. */
function isIntelAutomationPackComplete(rows, period) {
  ...
  const inst = String(institutionalBriefKindForPeriod(period) || '').toLowerCase();
  return kinds.has(inst);
}

function shouldTriggerIntelGapFill({ period, briefsDate, autoGenerate }) {
  if (autoGenerate) return true;
  const key = `${period}:${briefsDate}`;
  const now = Date.now();
  const last = intelGapFillCooldown.get(key) || 0;
  if (now - last < INTEL_GAP_FILL_COOLDOWN_MS) return false;
  ...
  if (req.method === 'GET') {
    ...
    if (isIntel) {
      ...
      if (
        packIncomplete &&
        isTraderDeskAutomationConfigured() &&
        shouldTriggerIntelGapFill({ period, briefsDate: autogenAnchor, autoGenerate })
      ) {
        Promise.resolve()
