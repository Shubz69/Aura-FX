...
const { generateAndStoreBriefSet } = require('./services/autoBriefGenerator');
...
async function ensureAutoGeneratedBriefsIfMissing({ date, period, autoGenerate = true }) {
  ...
  try {
    const [rows] = await executeQuery(
      `SELECT COUNT(DISTINCT COALESCE(brief_kind, 'general')) AS kind_count
       FROM trader_deck_briefs
       WHERE date = ? AND period = ?`,
      [date, period]
    );
    distinctKinds = Number(rows?.[0]?.kind_count || 0);
  } catch (_) {
    distinctKinds = 0;
  }
  if (distinctKinds >= 9) return;
