...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    if (isIntel) {
      ...
      let rows = await readBriefRows();
      if (rows.length < 9 && autoGenerate) {
        // Never block this endpoint on auto-generation. Trigger in background and return fast.
        Promise.allSettled([
          ensureAutoGeneratedBriefsIfMissing({ date, period, autoGenerate: true }),
          ensureAutoGeneratedCategoryBriefsIfMissing({ date, period, autoGenerate: true }),
        ]).catch(() => {});
      }
