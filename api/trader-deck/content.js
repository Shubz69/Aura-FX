 ...
 * Default is no autogen on read — avoids regenerating on every page load. Saturday / post-21:00 Sunday may walk back
 * to a UK weekday for rows; see `briefsSourceDate` and `weekendFallback`. Before 21:00 London on Sunday, daily intel
 * returns an empty pack with `intelDailySundayHold` (no Friday titles on Sunday morning).
 */
...
/**
 * True when institutional sleeves for this desk date are not all on the current Markdown document body
 * (same markers as assembleDailyBriefPlain / assembleWeeklyWfaPlain). Drives intel gap-fill + cron re-entry.
 */
async function deskCategoryBriefsNeedPdfReshape(briefsDate, period) {
  ...
  try {
    if (period === 'weekly') {
      const ok = await weeklyInstitutionalBodiesMarkdownV2Complete(executeQuery, briefsDate);
      return !ok;
    }
    const ok = await dailyInstitutionalBodiesMarkdownV2Complete(executeQuery, briefsDate);
    return !ok;
  } catch (_) {
    return false;
  }
...
/** One in-flight institutional autogen per (period, desk date) — avoids stacking gap-fills when the UI polls. */
const intelAutogenInFlight = new Map();
...
function maybeLogIntelContentRead(payload, ctx) {
  ...
  console.log(
    ...
    JSON.stringify({
      ...
      intelDailySundayHold: Boolean(ctx.intelDailySundayHold),
    })
  );
}
...
async function ensureAutoGeneratedBriefsIfMissing({ date, period, autoGenerate = true }) {
  ...
  try {
    if (period === 'weekly') {
      ...
      if (Number(rows?.[0]?.c || 0) >= INSTITUTIONAL_WEEKLY_WFA_KINDS.length) {
        const v2 = await weeklyInstitutionalBodiesMarkdownV2Complete(executeQuery, date);
        if (v2) return;
      }
    } else {
      ...
      if (Number(rows?.[0]?.c || 0) >= INSTITUTIONAL_DAILY_WFA_KINDS.length) {
        const v2 = await dailyInstitutionalBodiesMarkdownV2Complete(executeQuery, date);
        if (v2) return;
      }
    }
  } catch (_) {
...
/** London Sunday week-open brief — same cron path, triggered from intel GET gap-fill when rows are missing. */
async function ensureSundayMarketOpenBriefIfMissing({ date, autoGenerate = true }) {
  if (!autoGenerate) return;
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(date || ''))) return;
  if (!isLondonSundayYmd(date)) return;
  if (!isTraderDeskAutomationConfigured()) return;
  const runDate = periodDateToRunDate('daily', date);
  try {
    await generateAndStoreSundayMarketOpenBriefOnly({
      runDate,
      timeZone: 'Europe/London',
    });
  } catch (_) {
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    if (isIntel) {
      ...
      const sundayMorningHold =
        period === 'daily' && londonSundayIntelDailyHoldActive(date) && rows.length === 0;

      if (sundayMorningHold) {
        briefsDate = date;
      } else if (
        period === 'daily' &&
        isLondonWeekendYmd(date) &&
        !isLondonSundayYmd(date) &&
        rows.length === 0
      ) {
        /** Saturday-only: Sunday daily intel uses its own storage date (week-open brief), not prior weekday rows. */
      ...
      const sundayGapEligible =
        isSundayDeskDaily &&
        packNeedsWork &&
        !sundayMorningHold &&
        isTraderDeskAutomationConfigured() &&
        shouldTriggerIntelGapFill(gapFillParams);
      if (sundayGapEligible) {
        const sunKey = `sunday-open:${date}`;
        if (!intelAutogenInFlight.has(sunKey)) {
          const p = ensureSundayMarketOpenBriefIfMissing({ date, autoGenerate: true })
            .catch(() => {})
            .finally(() => {
              intelAutogenInFlight.delete(sunKey);
            });
          intelAutogenInFlight.set(sunKey, p);
        }
      }
      if (
        packNeedsWork &&
        ...
        const autogenLockKey = `${period}:${autogenAnchor}`;
        if (!intelAutogenInFlight.has(autogenLockKey)) {
          const p = ensureAutoGeneratedBriefsIfMissing({
            date: autogenAnchor,
            period,
            autoGenerate: true,
          })
            .catch(() => {})
            .finally(() => {
              intelAutogenInFlight.delete(autogenLockKey);
            });
          intelAutogenInFlight.set(autogenLockKey, p);
        }
      }
      ...
      maybeLogIntelContentRead(payload, {
        ...
        weekendFallback,
        intelDailySundayHold: sundayMorningHold,
      });
