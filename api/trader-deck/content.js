 ...
 *
 * Intel GET (daily): automated briefs UK weekdays only. Sat/Sun with no rows for that date walks back
...
const { generateAndStoreInstitutionalBriefOnly, generateAndStoreBriefSet } = require('./services/autoBriefGenerator');
const {
  isLondonWeekendYmd,
...
async function ensureAutoGeneratedBriefsIfMissing({ date, period, autoGenerate = true }) {
  ...
  if (period === 'daily' && isLondonWeekendYmd(date)) return;
  if (!String(process.env.PERPLEXITY_AUTOMATION_MODEL || '').trim()) return;
...
async function ensureAutoGeneratedCategoryBriefsIfMissing({ date, period, autoGenerate = true }) {
  ...
  try {
    const [rows] = await executeQuery(
      `SELECT DISTINCT COALESCE(brief_kind, 'general') AS brief_kind
       FROM trader_deck_briefs
       WHERE date = ? AND period = ?`,
      [date, period]
    );
    const existingKinds = new Set((rows || []).map((r) => String(r?.brief_kind || '').toLowerCase()));
    const requiredKinds = ['stocks', 'indices', 'futures', 'forex', 'crypto', 'commodities', 'bonds', 'etfs'];
    const hasAllRequired = requiredKinds.every((kind) => existingKinds.has(kind));
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    if (isIntel) {
      ...
      if (rows.length < 9 && autoGenerate && !isLondonWeekendYmd(autogenAnchor)) {
        // Never block this endpoint on generation; backfill missing briefs in the background.
        Promise.allSettled([
          ensureAutoGeneratedBriefsIfMissing({ date: autogenAnchor, period, autoGenerate: true }),
          ensureAutoGeneratedCategoryBriefsIfMissing({ date: autogenAnchor, period, autoGenerate: true }),
        ]).catch(() => {});
      }
