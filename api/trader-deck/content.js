...
async function deskCategoryBriefsNeedPdfReshape(briefsDate, period) {
  ...
  /** Legacy markdown-era bodies vs PDF-aligned plain-text (March 2026+) */
  const marker =
    period === 'weekly'
  ...
  try {
    const [rows] = await executeQuery(
      ...
      [
        ...
        ...DESK_AUTOMATION_CATEGORY_KINDS,
        marker,
        period === 'weekly' ? 'Forward Outlook' : 'Key Developments',
      ]
    );
...
/** True when all eight desk sleeves plus seven institutional PDF briefs exist (daily or weekly). */
function isIntelAutomationPackComplete(rows, period) {
  const kinds = briefKindsSet(rows);
  if (!DESK_AUTOMATION_CATEGORY_KINDS.every((k) => kinds.has(k))) return false;
  if (period === 'weekly') {
    return INSTITUTIONAL_WEEKLY_WFA_KINDS.every((k) => kinds.has(String(k).toLowerCase()));
  }
  return INSTITUTIONAL_DAILY_WFA_KINDS.every((k) => kinds.has(String(k).toLowerCase()));
}
...
async function ensureAutoGeneratedBriefsIfMissing({ date, period, autoGenerate = true }) {
  ...
  if (!isTraderDeckAutomationConfigured()) {
...
async function ensureAutoGeneratedCategoryBriefsIfMissing({ date, period, autoGenerate = true }) {
  ...
  if (period !== 'daily' && period !== 'weekly') return;
  if (!isTraderDeckAutomationConfigured()) {
    console.warn("[trader-deck/content] intel autogen skipped: PERPLEXITY_API_KEY is not set (category briefs)");
    return;
  }
  try {
    const [rows] = await executeQuery(
    ...
    const existingKinds = new Set(
      (rows || [])
        .map((r) => canonicalDeskCategoryKind(String(r?.brief_kind || '').toLowerCase()))
...
const BRIEF_KIND_ORDER_SQL =
  ...
  "WHEN 'aura_institutional_daily_fixed_income' THEN 0 " +
  "WHEN 'aura_institutional_daily_equities' THEN 0 " +
  ...
  "WHEN 'aura_institutional_weekly_fixed_income' THEN 1 " +
  "WHEN 'aura_institutional_weekly_equities' THEN 1 " +
  ...
  if (req.method === 'GET') {
    ...
    if (isIntel) {
      ...
      const packIncomplete =
        rows.length < expectedRows || !isIntelAutomationPackComplete(rows, period);
      ...
      if (
        ...
        const runDate = periodDateToRunDate(period, autogenAnchor);
        Promise.resolve()
          .then(() => ensureAutoGeneratedBriefsIfMissing({ date: autogenAnchor, period, autoGenerate: true }))
          .then(async () => {
            if (needsPdfReshape) {
              await generateAndStoreBriefSet({
                period,
                runDate,
                timeZone: 'Europe/London',
                isolateOutboundMeter: true,
              ...
              await ensureAutoGeneratedCategoryBriefsIfMissing({
                date: autogenAnchor,
                period,
                autoGenerate: true,
              });
      ...
      const briefs = (rows || []).map((r) => ({
        id: r.id,
        title: r.title || 'Brief',
        briefKind: canonicalDeskCategoryKind(String(r.brief_kind || '').toLowerCase()),
      ...
      const haveKind = new Set((rows || []).map((r) => canonicalDeskCategoryKind(String(r?.brief_kind || '').toLowerCase())));
      const institutionalPdfComplete =
        period === 'weekly'
          ? INSTITUTIONAL_WEEKLY_WFA_KINDS.every((k) => haveKind.has(String(k).toLowerCase()))
          : INSTITUTIONAL_DAILY_WFA_KINDS.every((k) => haveKind.has(String(k).toLowerCase()));
      const payload = {
        success: true,
        briefs,
        date,
