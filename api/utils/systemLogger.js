/**
 * Server-side mirror of src/utils/systemLogger.js (CommonJS for api/*).
 * Keep behavior in sync when changing log format.
 */
...
function resolveRequestId(payload) {
  ...
  try {
    const { getAsyncRequestId } = require('./asyncRequestContext');
...
function isLogEnabled() {
  const v = getEnv('SYSTEM_LOG_ENABLED');
...
function minLevel() {
  const raw = (getEnv('SYSTEM_LOG_LEVEL') || 'info').toLowerCase();
...
function logInfo(module, action, payload) {
  emit('info', module, action, payload);
}

function logWarn(module, action, payload) {
  emit('warn', module, action, payload);
}

function logError(module, action, payload) {
